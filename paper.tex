...
Removing positivity from the validity direction remains open. Dorsman and Gardner explicitly note that their OI conditions guarantee positive head rates only~\cite[Appendix~C.2, p.~254]{DG}. Our finite checks in Section~\ref{sec:finite} find no tall recurrent states on the tested nonexceptional cycles under head-only service either, suggesting that this hypothesis may be removable. At $w=1$ the exceptional cycles are odd and fail the bipartite hypothesis; the arbitrary-graph one-swap bipartite problem remains unresolved. Theorem~7 and Eq.~(25) of~\cite[p.~240]{DG}, which concern short initial placement orders, are unaffected. Counterexample~2 of~\cite[p.~239]{DG} gives product-form failure outside the relevant partite regime.
...
Here admissible OI allocations have nonnegative prefix increments and positive singleton capacities~\cite[Definition~1, p.~210]{DG}. Heads therefore receive positive service, while other rates may vanish.

The 20 symmetries in $\Gamma$ combine $x\mapsto\pm x+k\pmod5$ with optional queue exchange. They preserve allowed events, but need not preserve rates. Table~\ref{tab:orbits} gives representatives $t_i$; each vector lists destination orbits for successive queue positions.
\begin{table}[ht]
\centering\small
...
\end{proposition}
\begin{proof}
The action is free because queue lengths differ and all labels are distinct. Shorter-queue length separates rows 1--3, 4--6, and 7--9; within each group an equivalence fixes two distinct labels and hence is the identity. These nine balanced orbits have $9\cdot20=180$ states, exhausting $\mathcal B_2$ by~\eqref{eq:size}.
...
\]
where $a(x)=4-x$ and $b$ combines $x\mapsto2-x$ with queue exchange. These involutions generate $\Gamma$: their product has order ten and $a$ is not one of its powers. Equivariance lifts the paths to every orbit member, proving communication within the orbit of $t_7$. The head walk
\[
...
The exact successors and symmetry witnesses are in \texttt{data/all\_position\_certificate.json}. The verifier checks them, all 72,000 event/symmetry identities on the full five-job space, both displayed paths, and forward/reverse head reachability.
\end{document}
