...
This is universal validity over allocations. At an exceptional length it asserts the existence of an allocation violating canonical stationarity, not failure for every allocation. It is not a criterion for stationarity of an individually specified allocation. The sufficiency direction holds already for $w\ge1$: every nonexceptional cycle, in particular every even cycle, has only short recurrent classes. A law started outside those classes may converge to a mixture of classwise stationary laws; one globally normalized $W$ is not asserted.

\paragraph{Relation to Conjecture 1.}
Theorem~7 and Eq.~(25) of~\cite[p.~240]{DG} concern short initial placement orders and are unaffected. Conjecture~1 on printed p.~241 proposes transience of tall states on a $(w+1)$-partite graph. For every $w\ge2$, $C_{2w+1}$ is three-colorable, and its color classes can be refined to exactly $w+1$ nonempty independent sets. The nonempty closed exceptional set therefore refutes that transience claim inside its partite regime, even if non-head rates vanish. The same coloring observation applies to the larger exceptional cycles. At $w=1$ these cycles are odd and not bipartite: they are not counterexamples under the conjecture's hypothesis. The arbitrary-graph one-swap bipartite problem remains unresolved by this work. Counterexample~2 of~\cite[p.~239]{DG} already gives product-form failure outside the relevant partite regime.

The proof proceeds from actual completions to exact orientation moves (Section~\ref{sec:quotient}), then from circular runs to terminal classes (Section~\ref{sec:runs}). Short-class balance proves the positive direction (Section~\ref{sec:positive}); complementary exceptional-family calculations prove sharpness (Section~\ref{sec:failures}). The five-job exclusion of all queue-wise factors, a short queue-length consequence, and finite verification follow in Sections~\ref{sec:certificate}--\ref{sec:finite}.
...
There are two complementary cases. The first exceptional length, $n=2w+1$, admits a uniform calculation with one distinguished class rate. At every larger exceptional length, identical unit rates at all positions already suffice.
...
\end{tabular}
\end{center}
For $w=3$, there are 3,248 height-four states in total: 1,120 are recurrent and 2,128 are transient. All 560 height-five and 112 height-six states are transient. Thus identifying a state as tall, or even as height $w+1$, does not by itself identify the recurrent region. Detailed component-size distributions are in \texttt{results/classification.json}. The all-position conclusions now also follow from Theorem~\ref{thm:classification}; these numerical counts and the C7 head-only classification remain separately computed results.
...
\end{tabular}
\end{center}
The checks cover closure, zero-replacement communication within each orientation, communication between orientations, unlimited stationary balance, and the two-flow identity. They compare two transition implementations and separately audit changed incoming events in the complete spaces at $w=2,3$. Explicit source words and orientation moves are checked through $w=50$. On the family $n=2w+1$, at $\theta=1$, the canonical weights pass full stationary balance on the tested balanced regions $w=2,3,4,5$. This is computational evidence, not an asserted all-$w$ stationarity theorem. These computations supplement, rather than replace, the uniform proof.


\subsection{Complete exceptional-class check on \texorpdfstring{$C_9$}{C9}}
The unit-rate check enumerates all 131,040 configurations in $\mathcal T_{9,2}$, with 18 orientations and 1,179,360 position events for each of the limited and unlimited generators. It checks closure and compares two transition implementations. Using the integer weights $\binom9{|c|}=9!W(c,d)$, every unlimited residual is zero. Limited residuals are nonzero at 16,560 states, with value $-3$ at the target of Section~\ref{sec:nine}, and sum to zero at each queue length. The gained and lost event lists are complete on this support. These numerical counts are reproduced in \texttt{results/nine\_job.json}; the general residual theorem is proved independently of them.

\section{Conclusion}\label{sec:conclusion}
On cycles, a finite swap budget has an exact structural effect: the recurrent classes are the short orientation fibers and, precisely at $n=2kw+1$, one exceptional class with a single extra edge among otherwise full runs. The operational-to-orientation equivalence makes this a classification of the original queues. It yields a sharp universal canonical boundary for $w\ge2$, with different counterexample allocations covering the first and larger exceptional cycles. The five-job certificate goes further by excluding all queue-wise factors for one explicit allocation, and the head-only argument proves stronger rate robustness for that class. Neither strengthening is claimed for every exceptional cycle.

The classification assumes distinct jobs on a simple cycle and positive service at every position. It does not classify arbitrary swapping graphs, repeated classes, or general supports with vanishing non-head rates. The one-swap bipartite problem, allocation-specific stationarity on exceptional classes, and quantitative convergence rates remain directions for future work. No claim concerning the Markov-modulated Conjecture~2 of~\cite{DG} is made.

\section{Verification and reproducibility}\label{sec:verification}
The package includes editable \texttt{paper.tex}, the rendered PDF, every Lean source, exact stationary and orbit certificates, graph inputs, expected JSON results, and regeneration scripts. The standard-library command \texttt{python3 run\_checks.py} reconstructs the operational model and compares fresh exact outputs with the recorded finite results. It also runs under \texttt{-O}; verification does not depend on disabled assertions. The stationary certificate can be regenerated by rational elimination and independently checked against the full generator.

Lean verifies Theorems~\ref{thm:classification}, \ref{thm:sharp}, \ref{thm:balanced}, and~\ref{thm:unit}, the C9 specialization, the five-job certificate, and the continuous-time recurrence and queue-length assertions. It constructs the actual queue transitions, proves the quotient equivalence, and constructs measurable nonexplosive sample paths from exponential clocks. No external queueing theorem is installed as an axiom. Finite experiments retain their separate computational status. \texttt{LEAN.md} maps the mathematical statements to declarations with their precise assumptions; \texttt{verification/REPORT.md} records the checked revision, commands, audit results, and rendered-page inspection. The package instructions explain how to replay the complete verification and validate file hashes. Kernel checks use only Lean's standard logical axioms; no admitted proofs, custom axioms, or native-evaluation axioms are introduced.
...
Here an admissible OI allocation has nonnegative prefix increments and positive singleton capacities, as in~\cite[Definition~1, p.~210]{DG}; every nonempty queue consequently has a positive head rate. Non-head increments may vanish.
