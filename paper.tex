...
Comte and Dorsman~\cite{CD} introduced pass-and-swap (P\&S) queues as an extension of order-independent (OI) queues. Dorsman and Gardner~\cite{DG} proved that their canonical stationary law survives a swap limit when the initial placement order is short, and conjectured transience of taller configurations under a partiteness hypothesis. We classify recurrence for two queues in closed tandem with one distinct job per vertex of a cycle, and determine the sharp universal boundary for canonical stationarity.

The classification has a simple mechanism. Unlimited completions preserve the placement orientation, which records the relative order of compatible jobs~\cite[Lemma~1, p.~217]{DG}. On a cycle, truncating a replacement chain can reverse one edge (Lemma~\ref{lem:local}). Group consecutive cycle edges pointing in the same circular direction into \emph{runs}, and call a run \emph{full} when its length equals the budget $w$. An overfull run can transfer an excess edge to a neighboring run. Transport through full runs is reversible; filling a deficit or splitting a run is irreversible, by the run-count and deficit monotonicity proved in Section~\ref{sec:runs}. Under strictly positive position rates, the only tall recurrent pattern has one run of length $w+1$ and every other run full. It is exceptional precisely because transport is the only orientation change left. Its even number $2k$ of alternating runs forces $n=2kw+1$.

The classification determines whether truncation can remain active in a recurrent class. The balance calculations then determine when that activity defeats the canonical stationary weights. For $w\ge2$, at every exceptional length a positive OI allocation exposes a failure; outside those lengths the canonical law is stationary on every closed class for every positive OI allocation. The two counterexample calculations reveal different mechanisms at their target states: on $n=2w+1$, a distinguished class rate makes the gained and lost probability flows unequal; on larger exceptional cycles, the numbers of gained and lost events already differ, so unit rates suffice. Persistent truncation alone does not force failure for a specified allocation: the exact unit-rate check on $C_5$ at $w=2$ gives canonical stationarity despite continuing orientation changes (Section~\ref{sec:uniform-checks}).
...
\]
When $n=2kw+1$, $\gcd(n,w)=1$ and $n$ is odd. The first move has order $2n$: returning to the original edge requires a multiple of $n$ steps, and returning to its bit requires an even number. Every pair specifies a distinct orientation, since the long run is unique. The second move is the inverse of the first, and no other nontrivial moves are possible. Thus, for every $k\ge1$, the graph of nontrivial moves on the exceptional orientations is a bidirectional cycle on $2n$ vertices. This describes possible orientation changes, not transition rates. In particular, all exceptional orientations communicate; path lifting proves communication of all their extensions and cuts.
...
Conjecture~2 of~\cite[p.~245]{DG} proposes product form when an exogenous Markov process controls the swap limit in the tandem of its Section~5.2.2. Its statement has no short-initial-order restriction; that hypothesis appears explicitly in Theorem~7.1 of~\cite[p.~240]{DG}. We address Conjecture~2 as stated, including initial states in a tall closed class.

Keep $C_5$, one job per vertex, and the additive class rates $(2,1,1,1,1)$ in both queues. Independently of queue events, let the environment jump $L\to U$ and $U\to L$ at rate one. Mode $L$ uses budget two, and mode $U$ allows unlimited replacements; switching changes only the mode. On
\[
...
\paragraph{Unit-rate comparison.}
With unit service at every position and the same modulation, the stationary law on $\mathcal S$ is instead
\[
 \pi(c,d,b)=\frac{1}{16\,|c|!\,|d|!}
           =\pi_{\rm can}(c,d)\rho(b),\qquad \rho(L)=\rho(U)=\tfrac12.
\]
The exact finite check in \texttt{code/verify\_modulated.py} verifies that the canonical queue law balances both fixed-mode generators and their joint modulation. Thus class asymmetry matters for this five-job comparison; the larger exceptional cycles in Section~\ref{sec:larger} already fail at unit rates.
...
\subsection{Uniform-formula checks}\label{sec:uniform-checks}
