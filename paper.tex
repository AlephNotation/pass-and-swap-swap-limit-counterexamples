...
\paragraph{Formal verification.} Lean~4 checks nonemptiness, cardinality~\eqref{eq:size}, height $w+1$, and allocation-independent closure of $\B$ for all $w\ge2$; Lemmas~\ref{lem:irreducible} and~\ref{lem:two}; and the two-flow identity, balance formula, sign, and factorial defect~\eqref{eq:factorial}. It also checks a proper $(w+1)$-coloring and the finite-chain recurrence and stationary-law conclusions: return probabilities tend to one in the embedded completion chain, whose stationary equations are proved equivalent to the original generator equations. For an arbitrary finite completion kernel, closure yields at least one such recurrent balanced state. For $w=2$, Lean additionally checks the exact 180-state support, height, closure, head-communication, the $-1/360$ defect, the stationary certificate and its normalization, and nonfactorization over every characteristic-zero field. Continuous-time sample-path construction and the finite experiments in Section~\ref{sec:finite} are outside this formalization. The Lean closure proof proceeds directly through a rank invariant and does not use Lemma~1 of~\cite{DG}, so it constitutes an independent proof. The general balance proof also checks unlimited balance at $s_w$ directly. There are no admitted proofs or added mathematical axioms; \texttt{LEAN.md} records the declarations and verification commands.
